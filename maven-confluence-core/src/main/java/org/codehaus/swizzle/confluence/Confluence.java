...
*/
  ...
  /**
   * remove this token from the list of logged in tokens. Returns true if the user was logged out, false if they were
   * not logged in in the first place (we don't really need this return, but void
  ...
  /**
   * exports a Confluence instance and returns a String holding the URL for the download. The boolean argument
   * indicates
   * whether or not attachments ought to be included in the export.
   */
  ...
  /**
   * retrieve some basic information about the server being connected to. Useful for clients that need to turn certain
   * features on or off depending on the version of the server. (Since 1.0.3)
   */
  ...
  /**
   * exports a space and returns a String holding the URL for the download. The export type argument indicates whether
   * or not to export in XML, PDF, or HTML format - use "TYPE_XML", "TYPE_PDF", or
  ...
  /**
   * returns all the {@link PageSummary} instances in the space. Doesn't include pages which are in the Trash.
  ...
  /**
   * returns all the {@link PageHistorySummary} instances - useful for looking up the previous versions of a page, and
   * who changed them.
   */
  ...
  /**
   * returns all the {@link Attachment}s for this page (useful to point users to download them with the full file
  ...
  /**
   * add or update a page. For adding, the Page given as an argument should have space, title and content fields at a
   * minimum. For updating, the Page given should have id, space, title, content and
  ...
  /**
   * returns the HTML rendered content for this page. If 'content' is provided, then that is rendered as if it were the
   * body of the page (useful for a 'preview page' function). If it's not provided,
  ...
  /**
   * Like the above renderContent(), but you can supply an optional hash (map, dictionary, etc) containing additional
   * instructions for the renderer. Currently, only one such parameter is supported:
   */
  public String renderContent(String spaceKey, String pageId, String content, Map parameters) throws SwizzleException,
    ConfluenceException {
  ...
  /**
   * add a new attachment to a content entity object. *Note that this uses a lot of memory -- about 4 times the size of
   * the attachment.*
   */
  public Attachment addAttachment(long contentId, Attachment attachment, byte[] attachmentData) throws SwizzleException,
    ConfluenceException {
  ...
  /**
   * add or update a blog entry. For adding, the BlogEntry given as an argument should have space, title and content
   * fields at a minimum. For updating, the BlogEntry given should have id, space,
  ...
  /**
   * Retrieves a blog post in the Space with the given spaceKey, with the title 'postTitle' and posted on the day
   * 'dayOfMonth'.
   */
  public BlogEntry getBlogEntryByDayAndTitle(String spaceKey, int dayOfMonth, String postTitle) throws SwizzleException,
    ConfluenceException {
  ...
  /**
   * return a list of {@link SearchResult}s which match a given search query (including pages and other content types).
   * This is the same as a performing a parameterised search (see below) with an
   * empty parameter map.
   */
  ...
  /**
   * Returns a list of {@link SearchResult}s like the previous search, but you can optionally limit your search by
   * adding parameters to the parameter map. If you do not include a parameter, the
  ...
  /**
   * Returns a List of {@link Permission}s representing the permissions the current user has for this space (a list of
  ...
  /**
   * Returns a List of {@link Permission}s representing the permissions the given user has for this space. (since
   * 2.1.4)
   */
  ...
  /**
   * returns List of the space level {@link Permission}s which may be granted. This is a list of possible permissions
   * to
   * use with {{addPermissionToSpace}}, below, not a list of current permissions
   * on a Space.
   */
  ...
  /**
   * Give the entity named {{remoteEntityName}} (either a group or a user) the permission {{permission}} on the space
   * with the key {{spaceKey}}.
   */
  public boolean addPermissionToSpace(String permission, String remoteEntityName, String spaceKey) throws
    SwizzleException, ConfluenceException {
    Boolean value = (Boolean) call("addPermissionToSpace", permission, remoteEntityName, spaceKey);
    return value.booleanValue();
  }

  /**
   * Give the entity named {{remoteEntityName}} (either a group or a user) the permissions {{permissions}} on the space
   * with the key {{spaceKey}}.
   */
  public boolean addPermissionsToSpace(List permissions, String remoteEntityName, String spaceKey) throws
    SwizzleException, ConfluenceException {
  ...
  /**
   * Remove the permission {{permission} from the entity named {{remoteEntityName}} (either a group or a user) on the
   * space with the key {{spaceKey}}.
   */
  public boolean removePermissionFromSpace(String permission, String remoteEntityName, String spaceKey) throws
    SwizzleException, ConfluenceException {
  ...
  /**
   * remove a group. If {{defaultGroupName}} is specified, users belonging to {{groupname}} will be added to
   * {{defaultGroupName}}.
   */
  ...
  /**
   * Returns the {@link Label}s related to the given label name for the given {{spaceKey}}, with a specified maximum
   * number of results.
   */
   ...
   */
  public List getLabelsByDetail(String labelName, String namespace, String spaceKey, String owner) throws
    SwizzleException, ConfluenceException {
  ...
  private Object call(String servicePrefix, String command, Object[] args) throws SwizzleException {
    ...
    try {
      ...
      System.out.println(String.format("RPC ERROR - %s [%s]", servicePrefix, command));
